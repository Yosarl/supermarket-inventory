...
export default function LedgerAccountList() {
  ...
  useEffect(() => {
    ...
    ledgerAccountApi.list(companyId, undefined, searchTerm)
      .then((res) => {
        setLedgers((res.data.data as Ledger[]) ?? []);
      })
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setLedgers((prev) => prev.filter((l) => l._id !== deleteTargetId));
      setDeleteConfirmOpen(false);
      setDeleteTargetId(null);
      setSuccessDialogMessage('Ledger account deleted successfully');
      setSuccessDialogOpen(true);
    } catch (e) {
      setDeleteConfirmOpen(false);
      setDeleteTargetId(null);
      setErrorDialogMessage('Failed to delete ledger account');
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h4">Ledger Account List</Typography>
      </Box>
          ...
          <Table size="small">
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={12} align="center">No ledger accounts found</TableCell>
                </TableRow>
              ) : (
                ledgers.map((ledger, idx) => (
                  <TableRow key={ledger._id} hover>
                    ...
                    <TableCell>{ledger.mobile || '-'}</TableCell>
                    <TableCell>{ledger.phone || '-'}</TableCell>
                    <TableCell>{ledger.address || '-'}</TableCell>
                    <TableCell>{ledger.email || '-'}</TableCell>
                    <TableCell>{ledger.creditLimit ?? '-'}</TableCell>
                    <TableCell>{ledger.creditDays ?? '-'}</TableCell>
                    <TableCell align="center">
                      <IconButton
                        size="small"
                        color="primary"
                        onClick={() => navigate(`/master/other-ledger-create?edit=${ledger._id}`)}
                      >
